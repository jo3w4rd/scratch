...
static long handle_xu_operation(void *fh, bool valid_prio, struct uvc_xu_control_query *xu_query){
    ...
    int c;
    printk(KERN_ALERT "unit %u, sel %u, query %u, size %lu, dataP %p val %lu", xu_query->unit, xu_query->selector, xu_query->query, xu_query->size, xu_query->data, (__u16 *)(xu_query->data));
